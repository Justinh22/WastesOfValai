    ...
    def getInput(self):
        ...
        if self.game.keys["A"]:
            ...
            if self.state == "wandererSummary":
                newChar = self.game.directory.buildCharacter(self.game.player.party.level,self.game.player.party.members,self.game.player.getNewCharID())
        ...
        for i, enemy in enumerate(self.enemyList):
            ...
            if enemy.actReady:
                ...
                if enemy.coords == self.dungeonPos:
                    encounter = []
                    encounter = self.game.directory.buildEncounter(self.dungeonMap.dungeonLevel,self.dungeon.biome)
